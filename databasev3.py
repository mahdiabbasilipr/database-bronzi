...
#in version 3 i want to fix the eval problem whit \n in database
def get_Specifications (locate_of_poiter):# function goto get input frome user and return that in string that can change to dict by eval()
    #get Specifications of new pruduct and append them look like a dictonary
    print ("we need some imformation about your pruduct")
    ...
    print("\nyou\'r new product is :\nname = %s \nweight = %s \nhight = %s \nwidth = %s \nprice = %s "% (name,weight,hight,width,price))
    ...
    if user_approval == 'n' :#User approval chek
        ...
        if locate_of_poiter == 0:
            final_string = '{ \"code\" : %s ,\"name\" : \"%s\", \"weight\" : %s, \"hight\" : %s, \"width\" : %s, \"price\" : %s}'% (code,name,weight,hight,width,price)
        else :
            final_string = '\n{ \"code\" : %s ,\"name\" : \"%s\", \"weight\" : %s, \"hight\" : %s, \"width\" : %s, \"price\" : %s}'% (code,name,weight,hight,width,price)
    return final_string 
...
def get_list_data (addres_of_file):# make a list of dictionarys of datas 
    try:
        ...
        for strings in list_information:#if is a wrong data or synbol like \n in data 
            if len(strings) <=5 :
                list_information.remove(strings)#remove that to don't except bug in eval function 
...
def search_data (addres_of_file, code):#search a data by this code and return this dictonary 
    Reference_list = get_list_data(addres_of_file)
    #for arg in Reference_list :
    #    if arg["code"] == code :
    #        return arg                 
    ##################use lambda and ftlter
...
def print_data_in_TXT (addres_of_file, addres_of_final_file):#print data as addres_of_file to addres_of_final_file
    ...
    if user_approval_sorting == 'y':
        Ascending_Descending = int(input('send the number you wan\'t:\n1.descending\n2.ascending\n:'))#asking for now sorting descending or ascending
